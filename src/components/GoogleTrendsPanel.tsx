  ...
  return (
    <section>
      ...
      <div className="flex flex-col gap-1.5 mb-4">
        {reguas.map(([regua, itens]) => (
          <div key={regua} className="flex flex-wrap items-center gap-1.5">
            <span className="text-[8px] uppercase tracking-wider text-foreground/35 w-full sm:w-auto sm:min-w-[120px]">
              {itens.length > 1 ? `${itens.length} na mesma régua` : "sozinha"}
            </span>
            {itens.map(({ s: x, i }) => (
              <button
                key={`${regua}-${x.termo}`}
