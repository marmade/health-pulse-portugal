...
import { Trash2, Plus, Check, X, LogOut } from "lucide-react";
...
export default function Admin() {
  ...
  const fetchAll = async () => {
    const [kw, db, nw] = await Promise.all([
  ...
  const addKeyword = async () => {
    ...
    const { error } = await supabase.from("keywords").insert({
      ...
      axis: newKeyword.axis,
      is_active: newKeyword.is_active,
      category: newKeyword.axis,
      source: "manual",
      synonyms: [],
    });
    if (error) {
  ...
  const addDebunk = async () => {
    ...
    const { error } = await supabase.from("debunking").insert({
      title: newDebunk.title,
      term: newDebunk.term,
      source: newDebunk.source,
      classification: newDebunk.classification,
      url: newDebunk.url,
    });
    if (error) {
      toast({ title: "Erro ao guardar", variant: "destructive" });
  ...
  const addNews = async () => {
    ...
    const { error } = await supabase.from("news_items").insert({
      title: newNews.title,
      url: newNews.url,
      outlet: newNews.outlet,
      source_type: newNews.source_type,
      related_term: newNews.related_term || "geral",
      date: newNews.date,
    });
    if (error) {
      toast({ title: "Erro ao guardar", variant: "destructive" });
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center font-sans">
        <div className="w-full max-w-sm p-8">
          <h1 className="text-2xl font-bold text-[#0000FF] mb-2 tracking-tight">REPORTAGEM VIVA</h1>
          ...
          <div className="space-y-4">
            <Input
              ...
              className={`border-[#0000FF]/30 focus:border-[#0000FF] ${passwordError ? "border-red-500" : ""}`}
            />
            {passwordError && (
              <p className="text-sm text-red-500">Password incorrecta</p>
            )}
            <Button 
              onClick={handleLogin}
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      {/* Header */}
      <header className="border-b border-[#0000FF]/20 px-6 py-4 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-[#0000FF] tracking-tight">REPORTAGEM VIVA</h1>
        ...
        <Button
          ...
          className="border-[#0000FF]/30 text-[#0000FF] hover:bg-[#0000FF]/5"
        >
      ...
      <main className="p-6 max-w-6xl mx-auto">
        <Tabs defaultValue="keywords" className="w-full">
          <TabsList className="mb-6 bg-transparent border-b border-[#0000FF]/20 rounded-none w-full justify-start p-0 h-auto">
            <TabsTrigger 
              value="keywords" 
              className="rounded-none border-b-2 border-transparent data-[state=active]:border-[#0000FF] data-[state=active]:bg-transparent data-[state=active]:text-[#0000FF] px-4 py-2"
            >
              KEYWORDS
            </TabsTrigger>
            <TabsTrigger 
          ...
          <TabsContent value="keywords" className="mt-0">
            ...
            {showKeywordForm && (
              <div className="border border-[#0000FF]/20 p-4 mb-4 space-y-3">
                <Input
                  placeholder="Keyword"
                ...
                <div className="flex items-center gap-2">
                  <Switch
                    ...
                    onCheckedChange={(c) => setNewKeyword({ ...newKeyword, is_active: c })}
                  />
                  <span className="text-sm">Ativo</span>
                </div>
                <div className="flex gap-2">
                  <Button onClick={addKeyword} className="bg-[#0000FF] hover:bg-[#0000FF]/90" size="sm">
                    Guardar
                  </Button>
                  <Button onClick={() => setShowKeywordForm(false)} variant="outline" size="sm">
                    Cancelar
                  </Button>
                </div>
              </div>
            ...
            <div className="border border-[#0000FF]/20 max-h-[500px] overflow-auto">
              <Table>
                <TableHeader>
                  <TableRow className="border-b border-[#0000FF]/20">
                ...
                <TableBody>
                  {keywords.map((kw) => (
                    <TableRow key={kw.id} className="border-b border-[#0000FF]/10">
                      ...
                      <TableCell>
                        {deleteConfirm?.type === "keyword" && deleteConfirm.id === kw.id ? (
                          <div className="flex gap-1">
                            <Button size="sm" variant="destructive" onClick={() => deleteKeyword(kw.id)}>
                              Sim
                            </Button>
                          ...
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => setDeleteConfirm({ type: "keyword", id: kw.id })}
                          >
          ...
          <TabsContent value="debunking" className="mt-0">
            <div className="flex justify-between items-center mb-4">
              ...
              <Button
                onClick={() => setShowDebunkForm(!showDebunkForm)}
                className="bg-[#0000FF] hover:bg-[#0000FF]/90"
                size="sm"
              >
            ...
            {showDebunkForm && (
              <div className="border border-[#0000FF]/20 p-4 mb-4 space-y-3">
                <Input
                  ...
                  value={newDebunk.title}
                  onChange={(e) => setNewDebunk({ ...newDebunk, title: e.target.value })}
                />
                <Textarea
                  placeholder="Explicação / fact-check"
                  value={newDebunk.classification}
                  onChange={(e) => setNewDebunk({ ...newDebunk, classification: e.target.value })}
                />
                <Input
                  placeholder="Fonte (ex: DGS, 2024)"
                  value={newDebunk.source}
                  onChange={(e) => setNewDebunk({ ...newDebunk, source: e.target.value })}
                />
                <Input
                  placeholder="URL da fonte"
                  value={newDebunk.url}
                  onChange={(e) => setNewDebunk({ ...newDebunk, url: e.target.value })}
                />
                ...
                <div className="flex gap-2">
                  <Button onClick={addDebunk} className="bg-[#0000FF] hover:bg-[#0000FF]/90" size="sm">
                    Guardar
                  </Button>
                  <Button onClick={() => setShowDebunkForm(false)} variant="outline" size="sm">
                    Cancelar
                  </Button>
                </div>
              </div>
            ...
            <div className="border border-[#0000FF]/20 max-h-[500px] overflow-auto">
              <Table>
                ...
                <TableBody>
                  {debunking.map((item) => (
                    <TableRow key={item.id} className="border-b border-[#0000FF]/10">
                      ...
                      <TableCell>
                        {deleteConfirm?.type === "debunk" && deleteConfirm.id === item.id ? (
                          <div className="flex gap-1">
                            <Button size="sm" variant="destructive" onClick={() => deleteDebunk(item.id)}>
                              Sim
                            </Button>
                            <Button size="sm" variant="outline" onClick={() => setDeleteConfirm(null)}>
                              Não
                          ...
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => setDeleteConfirm({ type: "debunk", id: item.id })}
                          >
                            <Trash2 className="w-4 h-4 text-red-500" />
                          </Button>
                        )}
          ...
          <TabsContent value="news" className="mt-0">
            ...
            {showNewsForm && (
              <div className="border border-[#0000FF]/20 p-4 mb-4 space-y-3">
                ...
                />
                <Input
                  placeholder="Fonte (ex: Público, DGS)"
                ...
                />
                <div className="flex gap-2">
                  <Button onClick={addNews} className="bg-[#0000FF] hover:bg-[#0000FF]/90" size="sm">
                    Guardar
                  </Button>
                  <Button onClick={() => setShowNewsForm(false)} variant="outline" size="sm">
                    Cancelar
                  </Button>
                </div>
              </div>
