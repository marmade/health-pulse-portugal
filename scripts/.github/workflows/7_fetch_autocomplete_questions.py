...
pesadas por frequência histórica. NÃO inclui growth_percent (métrica não
disponível via autocomplete). Campo source="autocomplete" distingue estes
registos dos recolhidos pelo script 6_fetch_health_questions.py (pytrends).
...
# Pausa entre pedidos — sem quota declarada mas convém ser educado
...
# Seeds por keyword — cada template gera uma query de autocomplete.
# "{keyword}" é substituído pelo termo da keyword.
SEEDS_TEMPLATES = [
    "sintomas de {keyword}",
    "como tratar {keyword}",
    "o que é {keyword}",
...
def buscar_autocomplete(seed: str) -> list[str]:
    """
    Envia uma seed ao Google Autocomplete e devolve a lista de sugestões.
    client=firefox → resposta JSON simples sem callback.
    """
    try:
        r = requests.get(
            AUTOCOMPLETE_URL,
...
def buscar_perguntas_keyword(keyword: str, axis: str) -> list[dict]:
    """
    Agrega sugestões de autocomplete para todas as seeds desta keyword.
    relative_volume usa posição global como proxy de popularidade.
    growth_percent = 0 (não disponível via autocomplete).
    """
